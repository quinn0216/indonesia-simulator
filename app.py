...
def load_perfect_data():
    ...
    # 기온 변화량 열 찾기 - 'Change' 혹은 '기온' 텍스트 포함 확인
    change_col = None
    for c in df_temp.columns:
    ...
    try:
        ...
        # GDP 및 Poverty 컬럼 정확히 타겟팅 (띄어쓰기 유연성 확보)
...
with col1:
    ...
    # 컬럼 이름을 명확히 딕셔너리로 직접 지정하여 표 매핑 뒤틀림 방지
    res_df = pd.DataFrame({
        '순위': df['순위'],
